...
for(i in 1:n_rivers)
{
  ...
  if(name_river %in% comp_points$Nom.riv)
  {
    # computing the position of the empirical points in the elevation profile
    L[[i]][[13]] <- apply(X = st_distance(x = comp_points[comp_points$Nom.riv %in% name_river,],
    ...
    L[[i]][[10]] <- eval(substitute(
      ggplot()+
        ...
        geom_vline(aes(xintercept = L[[i]][[13]]*100,color = "Empirical CP"),linetype = "dashed")+
...
pdf(file = "partsection_wo_constraint.pdf",onefile = TRUE,paper = "a4")
for(i in 1:n_rivers)
{
  print(L[[i]][[12]])
}
...
save(RES,L,table_points,comp_points,file = "partsection_wo_constraint.Rdata")
# save(RES,file = "partsection_wo_constraint.Rdata")
write.table(x = RES,file = "partsection_wo_constraint.csv",sep = ";")
...
for(i in 1:nrow(tab_indic))
{
  ...
  tab_indic$Prec_2up_M1[i] <- abs(L[[L_key[i]]][[13]][1]-L[[L_key[i]]][[6]])*100
  tab_indic$Prec_2low_M1[i] <- abs(L[[L_key[i]]][[13]][2]-L[[L_key[i]]][[6]])*100
  tab_indic$Pts_order_M1[i] <- paste(c("Up_emp","Low_emp","CP1")[order(c(L[[L_key[i]]][[13]][1],L[[L_key[i]]][[13]][2],L[[L_key[i]]][[6]]))],collapse = "->")
  tab_indic$Prec_up2up_M2[i] <- abs(L[[L_key[i]]][[13]][1]-L[[L_key[i]]][[8]][1])*100
  tab_indic$Prec_low2low_M2[i] <- abs(L[[L_key[i]]][[13]][2]-L[[L_key[i]]][[8]][2])*100
  tab_indic$Overlap_est_over_emp_M2[i] <- round(length(which(L[[L_key[i]]][[13]][1]:L[[L_key[i]]][[13]][2] %in% L[[L_key[i]]][[8]][1]:L[[L_key[i]]][[8]][2]))/length(L[[L_key[i]]][[13]][1]:L[[L_key[i]]][[13]][2])*100,2)
  tab_indic$Overlap_emp_over_est_M2[i] <- round(length(which(L[[L_key[i]]][[8]][1]:L[[L_key[i]]][[8]][2] %in% L[[L_key[i]]][[13]][1]:L[[L_key[i]]][[13]][2]))/length(L[[L_key[i]]][[8]][1]:L[[L_key[i]]][[8]][2])*100,2)
  tab_indic$Pts_order_M2[i] <- paste(c("Up_emp","Low_emp","Up_CP2","Low_CP2")[order(c(L[[L_key[i]]][[13]][1],L[[L_key[i]]][[13]][2],L[[L_key[i]]][[8]][1],L[[L_key[i]]][[8]][2]))],collapse = "->")
  tab_indic$pos_up_emp[i] <- L[[L_key[i]]][[13]][1]
  tab_indic$pos_low_emp[i] <- L[[L_key[i]]][[13]][2]
...
# saving the results
save(tab_indic,file = "Indic_partsection_wo_constraint.Rdata")
